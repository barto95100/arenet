...
| R.3 | IA reorganisation (mostly subsumed by R.2) | Sidebar groups: **Aperçu** (Dashboard, Topology, Map-deferred), **Trafic** (Routes, Logs), **Sécurité** (WAF, Security, Certificates), **Administration** (Utilisateurs, Settings). `/certs` promoted out of `/settings/certificates` to a top-level route. `/waf` vs `/security` split confirmed per D4 reframing (both config surfaces, scoped differently). Map nav item present but routes to a "Coming soon" placeholder. **Note (2026-06-01)**: the structural IA work landed in R.2 alongside the chrome (Sidebar with the 4-section grouping + 6 stub routes for the new targets). What remains for R.3 proper is verification + spec alignment; the legacy-route redirects + the `/security` page content repurpose ship with R.4 alongside the per-page migration to avoid loop risk with the stubs. |
...
- Migration alias: SvelteKit redirects `/admin/users` → `/users`, `/observability` → `/dashboard`, `/settings/certificates` → `/certs` (per D5 safety net) ship in **R.4 alongside the content migration**, NOT in R.3. Adding the redirects in R.3 (before R.4 moves the content) would create a loop: the R.2 stubs at the new locations point operators at the legacy routes for the real content ("La source actuelle reste accessible sur /observability"), so a redirect from /observability back to /dashboard would round-trip the user through the stub indefinitely. R.4 migrates the content into the new routes FIRST, removes the "still accessible at /old" hint from the stubs, THEN adds the legacy-route redirects. `/security/decisions` is NOT redirected — it stays at its current path per D8 (invisible in sidebar but reachable by URL).
