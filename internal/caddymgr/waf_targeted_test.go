...
func TestBuildWAFHandler_TargetedExclusions_BeforeCRSAfterSecAction(t *testing.T) {
	got := buildWAFHandler("r-1", "example.com", "block", false, false, []int{920170}, nil,
		[]storage.WAFTargetedExclusion{{RuleID: 942100, Target: "ARGS:content"}})
...
func TestBuildWAFHandler_NoTargeted_Unchanged(t *testing.T) {
	a := buildWAFHandler("r-1", "example.com", "block", false, false, []int{942100}, nil, nil)
	b := buildWAFHandler("r-1", "example.com", "block", false, false, []int{942100}, nil, []storage.WAFTargetedExclusion{})
...
func TestBuildWAFHandler_TargetedOrderIndependent(t *testing.T) {
	...
	a := buildWAFHandler("r-1", "h", "block", false, false, nil, nil, []storage.WAFTargetedExclusion{x, y})
	b := buildWAFHandler("r-1", "h", "block", false, false, nil, nil, []storage.WAFTargetedExclusion{y, x})
	if a["directives"] != b["directives"] {
...
func runTargeted(t *testing.T, targeted []storage.WAFTargetedExclusion, url string) *targetedCaptureSink {
	t.Helper()
	cfg := buildWAFHandler("r-e2e", "example.com", "block", false, false, nil, nil, targeted)
